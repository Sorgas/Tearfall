		...
		for (int x = minX; x <= maxX; x++) {
			for (int y = minY; y <= maxY; y++) {
				if (slope.isInside(x, y)) {
					...
					if (container.getValleyElevation(x, y) > z) {
						container.setValleyElevation(x, y, Math.round(z));
					} else {
						if (container.getValleyElevation(x, y) == 0) {
							container.setValleyElevation(x, y, Math.round(z));
						}
					}
				}
	...
	private void smoothValleys(int iterations) {
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				elevationBuffer[x][y] = container.getValleyElevation(x, y);
			}
		}
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				container.setValleyElevation(x, y, Math.round(elevationBuffer[x][y]));
			}
		}
	}
