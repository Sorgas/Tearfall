 ...
 ******************************************************************************/
package stonering.generators.worldgen.voronoi.j2d;

import stonering.generators.worldgen.voronoi.convexClip.cVertex;
import stonering.generators.worldgen.voronoi.helper.Geometry;
import stonering.generators.worldgen.voronoi.convexClip.ConvexClip;
import stonering.generators.worldgen.voronoi.convexClip.cVertexList;
 ...
 */
	...
	public PolygonSimple convexClip(PolygonSimple poly) {
		...
		if (!this.getBounds2D().intersects(poly.getBounds2D()))
			return null;
		//checkItems if bounding box corners are in polygon: then poly is contained completely inside the outer polygon
		...
		//no intersection between the two polygons, so checkItems if one is inside the other
